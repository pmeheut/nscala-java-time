...
class RichLong(val underlying: Long) extends AnyVal with PimpedType[Long] {
  ...
  def toDateTime = ZonedDateTime.ofInstant(Instant.ofEpochMilli(underlying), ZoneId.systemDefault())

  def toDuration = Duration.of(underlying, ChronoUnit.MILLIS)

  def -(duration: Duration): Duration = toDuration.minus(duration)

  def +(duration: Duration): Duration = toDuration.plus(duration)
